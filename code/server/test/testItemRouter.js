...
function clearall(){
  agent.delete("/api/deleteAllItems")
    .then(function (res) {
      res.should.have.status(204);
      agent.delete("/api/deleteAllUsers")
      .then(function (res) {
        res.should.have.status(204);
...
function createALL(expectedHTTPStatus,id,description,price,skuid,supplierid){
...
function get_id(expectedHTTPStatus,id){
  it("get items", function (done) {
    agent.get("/api/items/" + id.toString()).send(
      ).then(function (res){
        res.should.have.status(expectedHTTPStatus);
        done();
      })})}
...
function delete_id(expectedHTTPStatus,id){
  it("get items", function (done) {
    agent.delete("/api/items/" + id.toString()).send(
      ).then(function (res){
        res.should.have.status(expectedHTTPStatus);
        done();
      })})}