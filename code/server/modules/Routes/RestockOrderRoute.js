...
router.post(
  ...
  async (req, res) => {
    const orderId = await sic.createRestockOrder(
      req.body.issueDate,
      req.body.supplierId,
      req.body.products
    );
...
router.put(
  ...
  async (req, res) => {
    const orderId = await sic.changeStateOfRestockOrder(
      req.params.id,
      req.body.newState
    );
    if (!orderId) {
...
router.put(
  ...
  async (req, res) => {
    const orderId = await sic.addSkuItemsToRestockOrder(
      req.params.id,
      req.body.skuItems
    );
    if (!orderId) {
...
router.put(
  ...
  [
    ...
    check("transportNote.deliveryDate")
      .notEmpty()
      .custom((d) => dayjs(d)),
  ],
  (req, res, next) => {
  ...
  async (req, res) => {
    const orderId = await sic.addTransportNoteToRestockOrder(
      req.params.id,
