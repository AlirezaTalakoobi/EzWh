...
async function testGetInternalAllOrders(){
    test("getInternalOrders", async () => {
        const user = await uc.newUser("Michael", "Jordan", "mj@ezwh.com", "customer", "MJtheGOAT");
        ...
        const order1 = await ic. createInternalOrder("2021/11/29 09:33", user.id, [{SKUId: sku2.id, description: "New super cool mountain bike: CANYON STOIC 4", price: 1789, qty: 2}]);
        const order2 = await ic. createInternalOrder("2022/05/05 09:33", user.id, [{SKUId: sku1.id, description: "Sweet restock of requested Acer Aspire 3", price: 237, qty: 1}]);
        ...
        const expectedOrders = [{
            ...
            state: "ISSUED",
            customerId: user.id,
            products: [{SKUId: sku2.id, description: "New super cool mountain bike: CANYON STOIC 4", price: 1789, qty: 2}]
          }, {
            id: order2,
            issueDate: "2022/05/05 09:33",
            state: "ISSUED",
            customerId: user.id,
            products: [{SKUId: sku1.id, description: "Sweet restock of requested Acer Aspire 3", price: 237, qty: 1}]
          }];
...
async function testCreateInternalOrderInvalidProducts(){
    test("createRestockOrderInvalidProducts", async () => {
        const user = await uc.newUser("Michael", "Jordan", "mj@ezwh.com", "customer", "MJtheGOAT");
        const sku2 = await sc.newSKU({description: "Canyon Stoic 4", weight: 14000, volume: 400, price: 1789, availableQuantity: 10});
        const order1 = await ic. createInternalOrder("2021/11/29 09:33", user.id, [{SKUId: sku2.id + 1, description: "New super cool mountain bike: CANYON STOIC 4", price: 1789, qty: 2}]);
...
async function testCreateInternalOrderInvalidCustomer(){
    test("createRestockOrderInvalidCustomer", async () => {
        const user = await uc.newUser("Michael", "Jordan", "mj@ezwh.com", "customer", "MJtheGOAT");
        const sku2 = await sc.newSKU({description: "Canyon Stoic 4", weight: 14000, volume: 400, price: 1789, availableQuantity: 10});
        const order1 = await ic. createInternalOrder("2021/11/29 09:33", user.id + 1, [{SKUId: sku2.id, description: "New super cool mountain bike: CANYON STOIC 4", price: 1789, qty: 2}]);
...
async function testModifyStateOfInternalOrderCompleted() {
    test("changeStateOfRestockOrderToCompleted", async () => {
        const user = await uc.newUser("Michael", "Jordan", "mj@ezwh.com", "customer", "MJtheGOAT");
        const sku1 = await sc.newSKU({description: "Acer Aspire 3", weight: 100, volume: 20, price: 237, availableQuantity: 15});
        const order2 = await ic. createInternalOrder("2022/05/05 09:33", user.id, [{SKUId: sku1.id, description: "Sweet restock of requested Acer Aspire 3", price: 237, qty: 2}]);
        ...
        const expectedOrder = {
            ...
            state: "COMPLETED",
            customerId: user.id,
            products: [{SKUId: sku1.id, description: "Sweet restock of requested Acer Aspire 3", price: 237, RFID: skuItems[0].RFID}, {SKUId: sku1.id, description: "Sweet restock of requested Acer Aspire 3", price: 237, RFID: skuItems[1].RFID}]
        };
