...
class TDController {
  ...
  editTDbyid = async (Body, ID) => {
    ...
    if (test === undefined) {
      ...
      try {
        let data = Body;
        const sql =
          "update TEST_DESCRIPTOR set name=?, procedureDescription=?, skuID=? where ID=?";
        const r = await this.dao.get("Select * from SKU where ID=?", [
          data.newIdSKU,
        ]);
  ...
  deleteTD = async (ID) => {
    if (
      (await this.dao.get("Select * from TEST_DESCRIPTOR where ID=?", [
        [ID],
      ])) !== undefined
    ) {
      try {
